 ...
 */
public class FilterSteps extends BaseNapSteps {
    ...
    @When("^I filter my wishlist items by (All|Accessories|Bags|Beauty|Clothing|Lingerie|Shoes)$")
    public void filterItemsByCategory(String filter) throws Throwable {

        wishListV3Page.clickFilterCategory(filter);

        int i=0;
        System.out.print("Waiting for '"+filter+"' filter to load");
    ...
    @And("^the filter option (All|Accessories|Bags|Beauty|Clothing|Lingerie|Shoes) is underlined$")
    public void assertFilterOptionSelected(String filter) throws Throwable {
        assertThat("The expected selected filter option did not match the actual selected filter option",
                wishListV3Page.getSelectedFilterElement().getText(), equalTo(filter.toUpperCase()));
    }

    @And("^the correct text is displayed for the empty filtered wishlist$")
    ...
    @And("^the correct text is displayed for the empty filtered All Items list$")
    ...
    @And("^The filter menu bar is displayed correctly$")
    ...
    @And("^Filtering by each filter category only displays items in that category$")
    public void assertAllFilterCategoriesItems() throws Throwable{
        ...
        for (ProductDsl.ProductCategory category : skuMap.keySet()) {
            ...
            expectedWishlistItemList = convertSizeToLowercase(expectedWishlistItemList);
    ...
    @And("^each filter option is underlined after clicking it$")
